...
# 📖 Project Overview
Beli 프로젝트는 수많은 객체(Boids)가 유기적으로 무리 지어 이동하는 군집 시뮬레이션을 언리얼 엔진(Unreal Engine) 환경에서 C++로 구현하는 프로젝트입니다. 단순한 로직 구현을 넘어, 수천에서 수만 개의 객체를 렌더링하고 연산하기 위한 최적화 기법을 점진적으로 적용하고 연구하는 것을 목표로 합니다.<br><br>
...
## ✅ Phase 1:
Basic Flocking System (Current)Boids 알고리즘 기초 구현<br>
크레이그 레이놀즈(Craig Reynolds)의 3대 규칙(Separation, Alignment, Cohesion) 적용.<br>
Instanced Static Mesh (ISM) 도입: 개별 액터를 스폰하는 대신, ISM 컴포넌트(UInstancedStaticMeshComponent)를 사용하여 드로우 콜(Draw Call)을 획기적으로 줄이고 수천 개의 기본 메시(Cone 등)를 렌더링.<br>
C++ 기반의 코어 연산 설계.<br>

## 🛠️ Phase 2: 
Performance Optimization (Planned)$O(N^2)$ 시간 복잡도를 가지는 거리 계산 로직의 최적화.공간 분할(Spatial Partitioning)<br>
Grid, Quadtree, 또는 Octree 구조를 도입하여 불필요한 이웃 탐색 연산 비용 감소.멀티스레딩(Multi-threading)을 활용한 병렬 처리 연산 검토.<br>

## ✨ Phase 3:
Advanced Rendering & Animation (Planned)VAT (Vertex Animation Texture) 적용<br>
스켈레탈 메시(Skeletal Mesh)의 무거운 본(Bone) 연산을 배제하고, 텍스처 데이터를 이용해 날갯짓 등의 유기적인 애니메이션을 가볍게 인스턴싱.<br>
